...
SoldierDiaryOverviewState::SoldierDiaryOverviewState(Base *base, size_t soldierId, SoldierInfoState *soldierInfoState) : _base(base), _soldierId(soldierId), _soldierInfoState(soldierInfoState)
{
	...
	if (_base == 0)
	{
		...
		add(_txtDeathInfo, "text1", "soldierDiary");
		add(_txtDeathDate, "text1", "soldierDiary");
	}
...
void SoldierDiaryOverviewState::init()
{
	...
	if ((_base == 0) && _soldier->getDeath())
	{
		std::string deathTitleText;
		std::ostringstream deathInfoText;
		...
		if (cause)
		{
			...
			deathInfoText << cause->getUnitName(_game->getLanguage());
			deathInfoText << " - ";
			deathInfoText << tr(cause->weapon);
		}
		else
		{
		...
		_txtDeathInfo->setText(deathInfoText.str());
	}
